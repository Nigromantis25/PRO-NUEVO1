...
- Se añadió botón "Login" en la barra de navegación.
- Se añadió un modal de inicio de sesión en `index.html` (id: `loginModal`).
- Se creó `js/login.js` (cliente demo). Credenciales demo: demo@demo.com / demo123

Cómo probar:
1) Abrir `index.html` en el navegador (doble clic o servidor local).
2) Hacer clic en "Login" y usar las credenciales demo indicadas.
3) Si el login es exitoso, el nombre/email aparecen en el botón y la sesión se guarda en localStorage.
4) Hacer clic en el nombre para cerrar sesión.

Siguientes pasos para producción:
- Reemplazar la validación cliente por llamadas a un backend (API REST) que autentique usuarios.
- Usar HTTPS, gestionar tokens (JWT) y almacenar sólo lo necesario (no contraseñas) en el cliente.
- Alternativa rápida: integrar Firebase Authentication para manejo seguro de usuarios sin backend propio.
